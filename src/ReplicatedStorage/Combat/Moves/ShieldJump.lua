...
function ShieldJump:Activate(player, character, rootPart, placementCFrame, class, classData, moveType)
    ...
    local distanceRay = workspace:Raycast(startCFrame.Position, startCFrame.LookVector * bezierData.distance, rayparams)
    if distanceRay then
        local rayPosition = distanceRay.Position
        local newDistance = (rayPosition - startCFrame.Position).Magnitude - 2

        endCFrame = startCFrame * CFrame.new(0, 0, -newDistance)
    end

    local ray = workspace:Raycast(endCFrame.Position, endCFrame.UpVector * -1000, rayparams)
    if ray then
        local floorPosition = ray.Position

        endCFrame = CFrame.lookAt(floorPosition, floorPosition + endCFrame.LookVector, Vector3.new(0, 1, 0))

        middleCFrame = startCFrame:Lerp(endCFrame, 0.5) * CFrame.new(0, 20, 0)

        bezierData.startCFrame = startCFrame
        bezierData.endCFrame = endCFrame * CFrame.new(0, 3, 0)
